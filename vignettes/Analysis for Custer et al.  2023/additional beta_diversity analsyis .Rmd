...
arabidopsis[arabidopsis$X %in% psT,]
...
core_abundance <- summarise_taxa(arabidopsis[arabidopsis$X %in% prrT,]) %>% summary() %>% data.frame() 
non_core_abundance <- summarise_taxa(arabidopsis[!(arabidopsis$X %in% prrT),]) %>% summary()
...
```{r, eval=FALSE}
psT<-sum_reads(human_stool, readn = 0.75)
...
```{r, eval=FALSE}
prT<-prop_reps(human_stool, prop_reps = 0.5)
...
```{r, eval=FALSE}
prrT<-prop_reads_and_reps(human_stool, prop_reps = 0.5, prop_reads = 0.0002)
