---
title: "Figure: Human Venn Diagram"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Figure: Human Venn Diagram}
...
# Load in phyloseq object from the github

This datsaet was used for comparison of core assignment methods. This human_ps phyloseq object contains all samples from the human microbiome project.
...
human_ps <- prune_taxa(taxa_sums(hmbp_working_set) >=1, hmbp_working_set)
```

```{#r, eval=FALSE}
githubURL <- ("https://raw.githubusercontent.com/MayaGans/CoreMicro/master/data-raw/human_ps.rda")
download.file(githubURL,"human_ps.rda", method="curl")
human_ps <- readRDS("human_ps.rda")
```

# Create subset 

For the manuscript, we used a subset of the above data, filtering to only include stool samples:
...
length(prop_seq_reads_core[!prop_seq_reads_core %in% human_node_list]) 
